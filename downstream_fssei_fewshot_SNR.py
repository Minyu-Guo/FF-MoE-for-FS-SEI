...
_TIME_PATTERNS = [
    # 20251130_235959 / 2025-11-30_23-59-59 / 2025.11.30-235959 etc
    re.compile(r'(?P<Y>\d{4})[-_.]?(?P<m>\d{2})[-_.]?(?P<d>\d{2})[T\s_-]?(?P<H>\d{2})[:\-_.]?(?P<M>\d{2})[:\-_.]?(?P<S>\d{2})'),
    # 20251130_2359 (no seconds)
    ...
    for enc in encodings:
        try:
            with open(path, "r", encoding=enc, newline="") as f:
                txt = f.read()
            # splitlines and parse via csv with possible delimiter
...
# ------------------------- reporting -------------------------
# def save_confusion_plots(cm, labels_str, out_prefix, n_way=None, n_shot=None):
...
#     # 根据是否传入 n_way/n_shot 组一个前缀
#     if (n_way is not None) and (n_shot is not None):
#         title_prefix = f"{n_way}-way {n_shot}-shot "
#     else:
#         title_prefix = ""
        
#     def _heatmap(mat, title, path, vmin=None, vmax=None):
#         plt.figure(figsize=(10, 8))
#         if sns is not None:
#             ax = sns.heatmap(mat, annot=False, cmap="Blues",
...
#     _heatmap(cm, f"{title_prefix} Confusion (Counts)", out_prefix + "_counts.png")
#     _heatmap(cm_norm, f"{title_prefix} Confusion (Row-normalized)", out_prefix + "_norm.png", vmin=0, vmax=1)
...
def save_confusion_plots(
    ...
    annot_min_count=1,         # 只标注 >= 这个阈值的 count
...
# # 生成并保存混淆矩阵（显示设备名称）
# def save_downstream_confusion(y_true, y_pred, class_names, save_path, rot=60):
#     if plt is None:
#         print("[Plot][Warn] matplotlib not available -> skip downstream confusion png.")
#         return
#     if sns is None:
#         print("[Plot][Warn] seaborn not available -> skip downstream confusion png.")
#         return

#     cm = confusion_matrix(y_true, y_pred, labels=np.arange(len(class_names)))
#     cmn = cm.astype(np.float64) / (cm.sum(axis=1, keepdims=True) + 1e-12)  # 行归一化
...
#     ax = sns.heatmap(cmn, cmap="Blues", vmin=0, vmax=1, 
#                 xticklabels=class_names, yticklabels=class_names)
    
#     # 关键：倾斜 + 对齐（更省空间）
#     ax.set_xticklabels(ax.get_xticklabels(), rotation=rot, ha="right", rotation_mode="anchor")
#     ax.set_yticklabels(ax.get_yticklabels(), rotation=0)
...
#     plt.xlabel("Pred")
#     plt.ylabel("True")
#     plt.title("Downstream VAL Confusion (Row-norm)")
#     plt.tight_layout()
...
def main():
    ...
    ap.add_argument(
        ...
        help="逗号分隔的 SNR(dB)，例如 'clean,30,25,20,15,10,5,0'，其中 clean/inf 表示不加噪声（基线）",
    )
    ap.add_argument(
        "--no_time_sep",
        action="store_true",
    ...
    ap.add_argument("--save_json", type=str, default="", help="可选：把E4-A summary保存成json")
    ...
    #     # 先看 NPZ 里的 class_names
    #     if cid < len(class_names):
    #         name = class_names[cid]
    #         if name is not None and str(name).strip() != "":
    #             return str(name)
    #     # 再看 label_map.csv 里的 name_map
    #     if cid in name_map and name_map[cid]:
    ...
    # [CLEANUP] 提前一次性加载 class_names，在 get_name() 和后面的混淆矩阵都用这一个
    raw_cn = Z.get("class_names", None)
    if raw_cn is None:
        class_names = []
    else:
    ...
    #         # 新增：校验gate_w是否全0
    #         if np.all(gate_w == 0):
    #             print(f"[ERROR] NPZ中的gate_w全为0！请检查NPZ文件：{args.npz}")
    #         else:
    #             print(f"[SUCCESS] 读取gate_w成功：shape={gate_w.shape}，均值={np.mean(gate_w):.4f}")
    #     else:
    #         print(f"[ERROR] gate_w维度错误：gw.shape={gw.shape}，N={N}")
    # else:
    #     print(f"[ERROR] NPZ文件中没有gate_w字段！字段列表：{list(Z.keys())}")
    if "gate_w" in Z:
        gw = np.array(Z["gate_w"]).astype(np.float32)
        if gw.ndim == 2 and gw.shape[0] == N:
            gate_w = gw
    ...
    def summarize_snr_curve(csv_path: str, out_json: str = None, worst_k_low_snr: int = 5):
        ...
        # # 过滤nan值（仅处理异常情况）
        # snr_vals = np.where(np.isnan(snr_vals), 35.0, snr_vals)
        # # 排序前确保无nan，避免排序异常
        # finite_mask = np.isfinite(snr_vals)
        # if not np.all(finite_mask):
        #     snr_vals[~finite_mask] = 35.0  # 兜底
        ...
        # 你这个 fewshot_snr_curve.csv 里只有 overall mean_acc，没有 per-class acc/混淆矩阵，
        # 所以无法真正算 worst-k（它需要每个类的 acc）。这里给提示 + None 占位。
    ...
    # print("[DBG] calling summarize_snr_curve, csv=", out_csv_snr)
    ...
    # one-time warning flag (avoid spamming "无gate_w数据" every episode)
    ...
    while ep < args.episodes and attempts < max_attempts:
        ...
        #     t_all = time.time()
        #     for snr_name, snr_db in snr_cfgs:
        #         t0 = time.time()
        #         print(f"[GateRaw] SNR={snr_name} begin")
        #         # 这里沿用 run_snr_sweep 的逻辑：
        #         # 如果 clean，则直接用 H；否则对 H 加噪得到 H_snr
        #         if snr_db is None:
        #             H_snr = H
        #         else:
        #             # 如果你已有专门的加噪函数，就调用你脚本/工具里的那一个
        #             # 下面这句要替换成你项目里 run_snr_sweep 实际使用的加噪方式
        #             H_snr = add_gaussian_noise_by_snr(H, snr_db, seed=args.seed)

        #         details = eval_fewshot_protonet_strict_set(
        #             H=H_snr,
        #             y=y,
        #             novel_classes=novel_classes,
        #             n_way=args.n_way,
        ...
        #         print(f"[GateRaw] SNR={snr_name} done, time={time.time()-t0:.1f}s")

        #         all_qry_idx = details.get("all_qry_idx", [])
        #         all_true = details.get("all_true", [])
        #         all_pred = details.get("all_pred", [])

        #         for epi_id, (qry_idx_ep, y_qry_ep, pred_qry_ep) in enumerate(zip(all_qry_idx, all_true, all_pred), start=1):
        #             current_gate = gate_w[np.asarray(qry_idx_ep)].copy()

        #             rows_this_ep = build_gate_raw_rows(
        #                 gate_qry=current_gate,
        #                 qry_idx=np.asarray(qry_idx_ep),
        #                 y_qry=np.asarray(y_qry_ep),
        #                 pred_qry=np.asarray(pred_qry_ep),
        ...
        #     print(f"[GateRaw] total time={time.time()-t_all:.1f}s")   
        # else:
        #     print("[Warn] 当前输入NPZ无gate_w，跳过 all-SNR gate raw records 收集。")

        # if gate_w is not None:
        #     # 仅在有 gate_w 时收集门控数据（MoE）
        #     current_gate = gate_w[qry_idx].copy()

        #     if np.all(current_gate == 0):
        #         print(f"[Warn] Episode {ep} 的 gate_w 全为0！")

        #     gate_buf.append(current_gate)
        ...
        #     gate_raw_rows.extend(rows_this_ep)
        # else:
        #     # DL baseline / non-MoE embedding 是正常情况：没有 gate_w
        #     if not warned_no_gate_w:
        #         print("[Warn] 当前输入NPZ无gate_w（非MoE导出embedding），将跳过GateDelta/门控可视化相关统计。")
        #         warned_no_gate_w = True
   ...
   # 加载NPZ时获取class_names
    ...
    eval_ids = list(map(int, eval_classes.tolist()))
    # 优先使用NPZ的class_names，若没有则用name_map兜底
    if len(class_names) > 0:
        # class_names是按类别ID顺序的设备名列表，直接根据eval_ids取对应名称
        labels_str = [class_names[cid] if cid < len(class_names) else get_name(cid) for cid in eval_ids]
    else:
        # 没有class_names时，用name_map生成
        labels_str = [get_name(cid) for cid in eval_ids]

    # save_confusion_plots(cm, labels_str=labels_str, out_prefix=out_cm_prefix, n_way=args.n_way,n_shot=args.n_shot)
    ...
    # === 同时保存一个 fewshot_cm_counts.csv，格式和老脚本一样 ===
    ...
    # # ==============调用你的 save_downstream_confusion 函数==============================
    # # 1. 拼接所有真实/预测标签
    # y_true = np.concatenate(true_buf)
    # y_pred = np.concatenate(pred_buf)

    # # 2. 获取类别名称（二选一）：
    # # 方式A：从NPZ直接加载（若export_hfusion_supcon.py保存了class_names）
    # class_names = Z.get("class_names", [class_label(cid, name_map) for cid in eval_ids])

    # # 方式B：从label_map生成（兼容原有逻辑）
    # # class_names = [name_map.get(cid, str(cid)) for cid in eval_ids]
    # # 3. 调用新函数保存混淆矩阵
    # save_downstream_confusion(
    #     y_true=y_true, 
    #     y_pred=y_pred, 
    #     class_names=class_names, 
    #     save_path=os.path.join(out_dir_final, "downstream_confusion_device_names.png")  # 用最终输出目录
    # )
    # print("Saved:", os.path.join(out_dir_final, "downstream_confusion_device_names.png"))
    # ========================================================================================

    # gate delta table (if gate_w exists)
    # ===== init optional gate-related buffers (for MoE only) =====
    # gate_all = None
    # true_all = None
    # pred_all = None
    ...
    # ============== 核心修正：Gate可视化调用（移到所有数据准备完成后） ==============
    if gate_all is not None and true_all is not None and pred_all is not None:
        # ===== ADD START: shot-level gate heatmap row =====
        # heatmap_row = compute_gate_mean_row(
        #     gate_qry=gate_all,
        #     sample_size=args.n_shot,
        #     expert_names=expert_names if len(expert_names) > 0 else [f"Expert{i}" for i in range(gate_all.shape[1])]
        # )
        ...
        #     df_heat = pd.DataFrame([heatmap_row])

        #     keep_cols = ["SampleSize", "Time", "Freq", "TF", "Inst"]
        #     for c in keep_cols:
        #         if c not in df_heat.columns:
        #             df_heat[c] = np.nan
        #     df_heat = df_heat[keep_cols]

        #     df_heat.to_csv(out_gate_heatmap, index=False)
        #     print("Saved:", out_gate_heatmap)
        # else:
            # print("[GateHeatmap] gate_all为空，跳过热力图表保存")
        # 动态获取类别数（替代硬编码16）
        num_eval_classes = len(eval_ids)  # 或用np.unique(true_all).size
        ...
        snr_labels = np.empty(len(true_all), dtype=np.float64)  # 替换zeros_like
        ...
        if os.path.exists(snr_details_path):
            ...
            if "snr_db" in df_snr_details.columns and len(df_snr_details) == len(true_all):
                # snr_labels = df_snr_details["snr_db"].values
                # snr_labels = np.where(snr_labels == "clean", 35.0, snr_labels)
                # snr_labels = snr_labels.astype(float)  # 转数值型
                snr_db_series = df_snr_details["snr_db"].astype(str).str.strip().str.lower()
                # 映射clean为35.0，其他值转为float（容错非数值）
                snr_labels = np.array([
    ...
    # ===== ADD END: gate raw + aggregated usage tables =====
