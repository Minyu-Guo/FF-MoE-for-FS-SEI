...
# -*- coding: utf-8 -*-
"""
train_moe_tfcnn_supcon_explain_closedloop.py
...
python train_moe_tfcnn_supcon_explain_closedloop_v2.py  --mat_all FeatureMatrix_3.mat   --topk_mat topk_indices.mat   --save_dir ablation/exp_no_inst     --init_from ./stage2_gate_only_occ/moe_injected_experts_occ.pth   --train_gate_only  --gate_align_level batch_mean  --disable_expert inst
...
#         self.X_topk = np.asarray(X_topk, dtype=np.float32)
#         self.fs_vec = np.asarray(fs_vec, dtype=np.float32)
#         self.X_spec = np.asarray(X_spec, dtype=np.float32)
#         self.X_occ = np.asarray(X_occ, dtype=np.float32)
...
#         j = self.indices[i]
#         return (
#             torch.from_numpy(self.X_all[j]).float(),
#             torch.from_numpy(self.X_topk[j]).float(),
#             torch.tensor([self.fs_vec[j]], dtype=torch.float32),
#             torch.from_numpy(self.X_spec[j]).float(),
#             torch.from_numpy(self.X_occ[j]).float(),
...
class FeatureFamilyMoE(nn.Module):
    ...
    def forward(self, x_all, x_topk, fs_feat, x_spec, x_occ):
        ...
        gate_in = torch.cat([x_all, x_topk, fs_feat], dim=1)    # 输入
...
def main():
    ...
    #     # 兼容：ckpt 可能就是纯 state_dict，也可能是 {"model": state_dict}
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        with torch.no_grad():
            for xb_all, xb_topk, xfs, xb_spec, xb_occ, yb in val_loader:
                ...
                # logits, gate_w, _, _ = model(xb_all, xb_topk, xfs, xb_spec)
                logits, gate_w, _, expert_logits, _ = model(xb_all, xb_topk, xfs, xb_spec, xb_occ)
                # logits, gate_w, expert_logits, h_fusion = model(xb_all, xb_topk, xfs, xb_spec, xb_occ)
        ...
        # eps = 1e-12
        # val_acc_contrib = (val_credit / max(1, val_n)).detach().cpu().numpy()       # [E] 和 val_acc 同量纲；每个专家对“总体 val_acc”贡献了多少（四个加起来=val_acc）
        # val_resp_acc    = (val_credit / (val_gate_mass + eps)).detach().cpu().numpy()  # [E]：当 gate 给它权重时，它带来正确的比例；专家“被用到时”表现好不好（类似 precision：gate 给它的权重里，有多少落在正确样本上）

        # print(f"acc_contrib={np.round(val_acc_contrib,3)} resp_acc={np.round(val_resp_acc,3)}")
        eps = 1e-12
        ...
        # for i, nm in enumerate(expert_names):
        #     row[f"val_acc_contrib_{nm}"] = float(val_acc_contrib[i])
        #     row[f"val_resp_acc_{nm}"]    = float(val_resp_acc[i])
    ...
    # ===== 下面这组是你 route/soft/exp 的统计量：一定要先初始化 =====
    best_val_n = 0
